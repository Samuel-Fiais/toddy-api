...
import {
  ExceptionMessages,
  ExceptionCodes,
} from "src/domain/constants/exception.constants";
...
export class ExceptionService implements IException {
  ...
  applicationNotFound(entity: string, id: string = "all"): void {
    throw new NotFoundException(`${ExceptionMessages.NOT_FOUND} : ${entity} : ${id}`);
  }
  ...
  applicationExistingRegister(properties: string[]) {
    throw new ConflictException(`${ExceptionMessages.EXISTING_REGISTER} : ${properties.join(", ")}`);
  }
  ...
  applicationValuesRequisitionInvalid(
    ...
    throw new BadRequestException(`${ExceptionMessages.VALUES_REQUISITION_INVALID} : ${entity} : ${yupErrorsString}`);
  }

  applicationOperationCreateRepository(entity: string) {
    throw new InternalServerErrorException(`${ExceptionMessages.OPERATION_CREATE_REPOSITORY} : ${entity}`);
  }

  applicationOperationUpdateRepository(entity: string) {
    throw new InternalServerErrorException(`${ExceptionMessages.OPERATION_UPDATE_REPOSITORY} : ${entity}`);
  }

  applicationOperationDeleteRepository(entity: string) {
    throw new InternalServerErrorException(`${ExceptionMessages.OPERATION_DELETE_REPOSITORY} : ${entity}`);
  }

  applicationOperationFindRepository(entity: string) {
    throw new InternalServerErrorException(`${ExceptionMessages.OPERATION_FIND_REPOSITORY} : ${entity}`);
  }
